...
def score_model(composed_regressor_params,data,cv=10,file_name="composed_model",shap=False): 
    ...
    """
    params_only_linear=[] #a copy of the parameters but setting non_linear_regressor to "None", used in linear model evaluation 
    for param in composed_regressor_params:
        param_only_linear=copy.deepcopy(param)
        param_only_linear["non_linear_regressor"]="None"
        params_only_linear.append(param_only_linear)
    """
    ...
    #borrame=sklearn.model_selection.cross_val_predict(new_regressor,data,data["pKa"],cv=10,n_jobs=n_jobs)
    ...
    #                                    scoring=scoring, cv=14,n_jobs=n_jobs,verbose=0)
    cross_validated_model=sklearn.model_selection.cross_validate(new_regressor,data,data["pKa"],scoring=scores, cv=cv_splitter_2,n_jobs=n_jobs,verbose=0,return_estimator=True)
    ...
    #train_pka_predictions=cross_validated_model["estimator"]
    #train_pka_predictions=cross_validated_model.estimator(data)
    #train_pka_predictions=sklearn.model_selection.cross_val_predict(new_regressor,data,data["pKa"],cv=cv_splitter,n_jobs=n_jobs)  
    train_predicted_residuals= train_pka_predictions - train_linear_pka_predictions   

    r2=cross_validated_model["test_r2"]
    neg_mean_squared_error=cross_validated_model["test_neg_mean_squared_error"]
    neg_mean_absolute_error=cross_validated_model["test_neg_mean_absolute_error"]
    n_instances=len(data["pKa"])
    chi_squared=-n_instances*neg_mean_squared_error.mean(axis=0)/(n_instances-len(all_features))
    aic=-(2*len(all_features)/n_instances+np.log(chi_squared))
    ...
    if shap:
        ...
        print ("shap results:")
        print (len(new_regressor_for_shap.non_linear_attributes))
        print (len(new_regressor_for_shap.shap_nl_feature_importances))
        for f,fi in  zip(new_regressor_for_shap.non_linear_attributes,new_regressor_for_shap.shap_nl_feature_importances):
            print (str(f)+ ": "+str(fi))
    ...
    pka_plot_trace=go.Scatter(x=train_pka_predictions,y=data["pKa"],mode='markers', showlegend=False,
    ...
    linear_root_mean_squared_error=np.mean((train_linear_pka_predictions-data["pKa"])**2)**.5
