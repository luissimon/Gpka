...
def get_lists_of_properties_for_symmetry_reduced_graph(molecules_HL,molecules,HL_molecules_file_names,ref_molecule):
    ...
    for pm,m,pmp,pNBOchgs,pNBOnbibo,pNBOwibergbo,NBO_NLMO_NPA_bo in zip(molecules_HL,molecules,multwfn_props,NBO_charges,NBO_nbi_bond_orders,NBO_wiberg_bond_orders,NBO_NLMO_NPA_bond_orders):
        ...
        bound_mask=rel_distances<1.25
        pmp["bond_distances"]=distances*bound_mask   #only distances smaler than 1.25 times the sum of covalent radius are kept; this is needed because distance is smaller for bound atoms than for other atoms
        pmp["rel_bond_distances"]=rel_distances*bound_mask #only rel-distances smaler than 1.25 are kept
        ...
        for key in atom_vector_properties:
            ...
            for i in range(len(groups_of_equivalent_atoms)):
                for j in range(len(groups_of_equivalent_atoms)):
                    ...
                    if "inv_distances"!=key:  
                        new_p[i,j]=np.sum( [ [ pmp[key][atom_i-1][atom_j-1] for atom_j in groups_of_equivalent_atoms[j]   ]   for atom_i in groups_of_equivalent_atoms[i] ]) #sum columns and rows
                    #for "inv_distances", instead of the sum use minimum value
                    else:
                        new_p[i,j]=np.min( [ [ pmp[key][atom_i-1][atom_j-1] for atom_j in groups_of_equivalent_atoms[j]   ]   for atom_i in groups_of_equivalent_atoms[i] ]) #sum columns and rows
