...
def get_lists_of_properties_for_symmetry_reduced_graph(molecules_HL,molecules,HL_molecules_file_names,ref_molecule):
    ...
    for pm,m,pmp,pNBOchgs,pNBOnbibo,pNBOwibergbo,NBO_NLMO_NPA_bo in zip(molecules_HL,molecules,multwfn_props,NBO_charges,NBO_nbi_bond_orders,NBO_wiberg_bond_orders,NBO_NLMO_NPA_bond_orders):
        ...
        print("distances_H")
        print(np.array(pmp["distances_H"]))
        print("nbo_charges")
        print(np.array(pmp["nbo_charges_H"]))
        ...
        for k in atom_scalar_properties+atom_vector_properties+atom_tensor_properties:
            ...
            pmp[k]=new_pmp
        #for k in ["distances_H"]:
        #    
        #    new_pmp=[pmp[k][i][:] for i in range(len(pm.atom_list)) if i not in h_numbers]
        #    pmp[k]=new_pmp
        print("distances_H after removing rows H atoms")
        print(np.array(pmp["distances_H"]))
        print("nbo_charges_H after removing rows of H atoms")
        print(np.array(pmp["nbo_charges_H"]))
        ...
        for k in atom_vector_properties:
            ...
            pmp[k]=new_pmp
        print("distances_H after removing columns H atoms")
        print(np.array(pmp["distances_H"]))
        print("nbo_charges_H after removing columns of H atoms")
        print(np.array(pmp["nbo_charges_H"]))
        ...
        print(groups_of_equivalent_atoms)
        print(connections_between_groups)
        ...
        #vector
        #sum columns
        #list that contains which of the atoms in every group of equivalent atoms is nearest to each atom 
        # (it does not contain the atom number of the nearest atom, but the index of the grous_of_equivalent_atoms list)
        #print([[[ [atom_i,atom_j]  for atom_j in eq_atoms ]   for eq_atoms in groups_of_equivalent_atoms]   for atom_i in range(len(pm.atom_list)) ])
        #for atom_i in range(len(pm.atom_list)):
        #     for eq_atoms in groups_of_equivalent_atoms:
        #          print("the nearest to atom:"+str(atom_i+1)+" in group "+str(eq_atoms)+" is:"+str(eq_atoms[np.argmin([pm.distance([atom_i+1,atom_j]) for atom_j in eq_atoms])]))
        index_nearest_in_group_for_every_atom=[[np.argmin([ pm.distance( [atom_i+1,atom_j])  for atom_j in eq_atoms ])   for eq_atoms in groups_of_equivalent_atoms]   for atom_i in range(len(pm.atom_list)) ]
        print("index_nearest_in_group_for_every_atom")
        print(index_nearest_in_group_for_every_atom)
        nearest_in_group_for_every_atom=[ [ g[z] for g,z in zip(groups_of_equivalent_atoms,atom) ] for atom in index_nearest_in_group_for_every_atom]
        print("nearest_in_group_for_every_atom")
        print(nearest_in_group_for_every_atom)
        index_nearest_in_group_for_every_atom=[[eq_atoms[np.argmin([ pm.distance( [atom_i+1,atom_j])  for atom_j in eq_atoms ])]   for eq_atoms in groups_of_equivalent_atoms]   for atom_i in range(len(pm.atom_list)) ]
        print(nearest_in_group_for_every_atom)
        for key in atom_vector_properties:
            ...
            for i in range(len(groups_of_equivalent_atoms)):
                for j in range(len(groups_of_equivalent_atoms)):
                    #print([[atom,nearest_in_group_for_every_atom[atom][j]] for atom in groups_of_equivalent_atoms[i]])
                   
                    new_p[i,j]=np.mean([pmp[key][atom-1][nearest_in_group_for_every_atom[atom-1][j]-1] for atom in groups_of_equivalent_atoms[i] ])
                    if key=="distances_H": print ([pmp[key][atom-1][nearest_in_group_for_every_atom[atom-1][j]-1] for atom in groups_of_equivalent_atoms[i] ])
            """
            new_p=[]
            for p in pmp[key]:
                new_p_vector=[]
                for eq_atoms in groups_of_equivalent_atoms:
                    ...
                    new_matrixp= (np.mean([    [p[eqa-1][index_nearest_in_group_for_every_atom[eqa-1][group]] for group in groups_of_equivalent_atoms ]    for eqa in eq_atoms]) )


                new_p.append(new_p_vector)
            """
            pmp[key]=new_p
        print("distances_H after combining by symmetry")
        print(np.array(pmp["distances_H"]))
        print("nbo_charges after combining by symmetry")
        print(np.array(pmp["nbo_charges_H"]))
        """
        #sum rows
        for key in atom_vector_properties:
...
for compn in rows:
    ...
    delete_features=[ "lea_atom_overall_surf_area_protonated", "lea_atom_overall_surf_area_deprotonated", "lea_atom_overall_surf_area_difference",# it is the same that was calculated for ESP 
                        ...
                        "rel_nbo_nbi_bond_orders_protonated","rel_nbo_nbi_bond_orders_deprotonated","rel_nbo_nbi_bond_orders_difference",
                        "rel_nbo_wiberg_bond_orders_protonated","rel_nbo_wiberg_bond_orders_deprotonated","rel_nbo_wiberg_bond_orders_difference",
                        "rel_nbo_nlmonpa_bond_orders_protonated","rel_nbo_nlmonpa_bond_orders_deprotonated","rel_nbo_nlmonpa_bond_orders_difference",               
                        ]
