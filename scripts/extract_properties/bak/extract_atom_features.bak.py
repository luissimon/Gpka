...
def get_lists_of_properties_for_symmetry_reduced_graph(molecules_HL,molecules,HL_molecules_file_names,ref_molecule):
    ...
    for pm,m,pmp,pNBOchgs,pNBOnbibo,pNBOwibergbo,NBO_NLMO_NPA_bo in zip(molecules_HL,molecules,multwfn_props,NBO_charges,NBO_nbi_bond_orders,NBO_wiberg_bond_orders,NBO_NLMO_NPA_bond_orders):
        ...
        for key in atom_scalar_properties:
            new_pmp_value=[]
            for eq_atoms in groups_of_equivalent_atoms:                                            # -1 because eqa starts at 1, not 0 
                if symmetry_method=="sum":          new_pmp_value.append(float(np.sum ( [ pmp[key][eqa-1] for eqa in eq_atoms  ])))
                elif symmetry_method=="mean":       new_pmp_value.append(float(np.mean( [ pmp[key][eqa-1] for eqa in eq_atoms  ])))
...
for compn in rows:
    ...
    #these atomic features were extracted but are useless, so they are deleted (maybe it could be possible to stop extracting them and save some time...)
    ...
    features_dict= {"feature_keys":[atom_features_publication_names[k] for k in atom_prop_scalar_keys] ,
                    "vector_keys": [vector_features_publication_names[k] for k in atom_prop_vector_keys]}  
