...
if level_of_theory not in ["M06","sM06","swb97xd","wb97xd","pbeh3c","r2scan3c","wb97x3c"]: print ("lot must be M06, sM06, swb97xd, wb97xd, pbeh3c or r2scan3c, wb97x3c"); sys.exit()
...
if do_sampl:
    ...
    values_extracted_file="values_extracted_sampl-"+weighting+"-"+level_of_theory+"."+output_suffix+".csv"
...
#the place where optimization output files live
...
one_water_route=output_files_route+"1water/"
...
if "-exclude" in sys.argv: 
    print(sys.argv[sys.argv.index("-exclude")+1])
    print(sys.argv[sys.argv.index("-exclude")+1] in os.listdir(os.getcwd()))
...
# ADD TERMS FOR DIFFERENT LEVELS OF THEORY!!!! 
if optimization_level_of_theory=="pbeh3c":
    ...
    ref_DG= (-76.81735937--76.40753369)   #= -0.40982568 hartrees; deltaG of H2O  --> H3O+; adding G(protonated)-G(desprotonated) yields deltaG for  protonated + H2O --> deprotonated + H3O+
    ref_SP= (-76.833787519213--76.410870286403)  #deltaSP of H2O  --> H3O+ ; 
    ref_zpE=ref_SP+(0.03554516-0.02099363)  #deltazPE of H2O  --> H3O+ ; 
...
elif HL_text=="_r2scan3c_chrg":
    ref_SP_HL= (-76.846637081625 --76.430883109420)  #deltaSP of H2O  --> H3O+ , r2scan3c level of theory
elif HL_text=="_wb97x3c_chrg":
    ref_SP_HL= (-17.700435130441 --17.283497112322)  #deltaSP of H2O  --> H3O+ , wb97x3c level of theory

nbo_text=HL_text[:-5]+"_nbo"
...
properties=[orbitals,SMD_energy,quadrupole_moment]
...
protonated_molecular_weight=pd.Series(dtype="float")
...
total_spatial_extent_protonated,total_spatial_extent_deprotonated=pd.Series(dtype="float"),pd.Series(dtype="float")
total_spatial_extent_difference=pd.Series(dtype="float")
...
counter=0
for compn in rows:  #cambiar
    counter+=1 
    if compn in exclude: continue 
    print(compn)
    ####################################       D E T E R M I N E    F I L E     S U F F I X E S    F R O M    E N T R Y    I N    D A T A B A S E       ########################################
    if str(compn.split("_")[1]).startswith("cation"):   protonated_str="-cation";  deprotonated_str="-neut";   protonated_charge[compn]=1; deprotonated_charge[compn]=0
    if str(compn.split("_")[1]).startswith("2cation"):  protonated_str="-2cation"; deprotonated_str="-cation"; protonated_charge[compn]=2; deprotonated_charge[compn]=1
    if str(compn.split("_")[1]).startswith("3cation"):  protonated_str="-3cation"; deprotonated_str="-2cation"; protonated_charge[compn]=3; deprotonated_charge[compn]=2
    if str(compn.split("_")[1]).startswith("4cation"):  protonated_str="-4cation"; deprotonated_str="-3cation"; protonated_charge[compn]=4; deprotonated_charge[compn]=3
    if str(compn.split("_")[1]).startswith("5cation"):  protonated_str="-5cation"; deprotonated_str="-4cation"; protonated_charge[compn]=5; deprotonated_charge[compn]=4
    if str(compn.split("_")[1]).startswith("6cation"):  protonated_str="-6cation"; deprotonated_str="-5cation"; protonated_charge[compn]=6; deprotonated_charge[compn]=5
    if str(compn.split("_")[1]).startswith("neut"):     protonated_str="-neut";    deprotonated_str="-an";     protonated_charge[compn]=0; deprotonated_charge[compn]=-1      
    if str(compn.split("_")[1]).startswith("an"):       protonated_str="-an";      deprotonated_str="-2an";    protonated_charge[compn]=-1;deprotonated_charge[compn]=-2
    if str(compn.split("_")[1]).startswith("2an"):      protonated_str="-2an";     deprotonated_str="-3an";    protonated_charge[compn]=-2;deprotonated_charge[compn]=-3
    if str(compn.split("_")[1]).startswith("3an"):      protonated_str="-3an";     deprotonated_str="-4an";    protonated_charge[compn]=-3;deprotonated_charge[compn]=-4
    if str(compn.split("_")[1]).startswith("4an"):      protonated_str="-4an";     deprotonated_str="-5an";    protonated_charge[compn]=-4;deprotonated_charge[compn]=-5
    if str(compn.split("_")[1]).startswith("5an"):      protonated_str="-5an";     deprotonated_str="-6an";    protonated_charge[compn]=-5;deprotonated_charge[compn]=-6
    #(...)
    ...
    #protonated_HL_molecules_file_names=[fn.split(".out")[0]+HL_text+".out" for fn in protonated_molecules_file_names]
    #deprotonated_HL_molecules_file_names=[fn.split(".out")[0]+HL_text+".out" for fn in deprotonated_molecules_file_names]
    ...
    ##################################       F O R    N O T     B L A N K      E N T R I E S    A N D      E X I S T I N G     F I L E S      ###################################################
        ...
        protonated_molecules=[Molecular_structure.Molecular_structure(f,"last",properties) for f in protonated_molecules_file_names]
        #for p in protonated_molecules: p.read_hess() #needed for the bond force constant
        deprotonated_molecules=[Molecular_structure.Molecular_structure(f,"last",properties) for f in deprotonated_molecules_file_names]
        #for d in deprotonated_molecules: d.read_hess()
        ...
        protonated_molecules_HL=[Molecular_structure.Molecular_structure(f,"last",properties) for f in protonated_HL_molecules_file_names]
        deprotonated_molecules_HL=[Molecular_structure.Molecular_structure(f,"last",properties) for f in deprotonated_HL_molecules_file_names]
        ...
        protonated_molecular_weight[compn]=np.sum( [protonated_molecules[0].atomic_weights[a.symbol.lower()] for a in protonated_molecules[0].atom_list  ])
        ...
        #################################         L I S T S      O F     E N E R G I E S     A N D       P O P U L A T I O N S           ########################################################
        #list of the energies, single point energies, gibbs correction of each conformer/tautomer for the protonated(deprotonated) 
        #corresponding to the entry in labels file compn with the level of theory used in the optimization
        #this must be read here since it is needed for calculating the populations. 
        # These lists will be replaced for tautomers, but the populations are needed before for averaging other properties
        protonated_molecules_energies=np.array([m.gibbs_free_energy for m in protonated_molecules])
        deprotonated_molecules_energies=np.array([m.gibbs_free_energy for m in deprotonated_molecules])
        protonated_sp_energies=np.array([m.electronic_energy for m in protonated_molecules])
        deprotonated_sp_energies=np.array([m.electronic_energy for m in deprotonated_molecules]) 
        protonated_molecules_gibbs_correction=protonated_molecules_energies-protonated_sp_energies
        deprotonated_molecules_gibbs_correction=deprotonated_molecules_energies-deprotonated_sp_energies
        protonated_molecules_zero_point_energies=np.array([m.zero_point_energy for m in protonated_molecules])
        deprotonated_molecules_zero_point_energies=np.array([m.zero_point_energy for m in deprotonated_molecules])
        protonated_molecules_zero_point_correction=protonated_molecules_zero_point_energies-protonated_sp_energies
        deprotonated_molecules_zero_point_correction=deprotonated_molecules_zero_point_energies-deprotonated_sp_energies        
        #list of the energies, single point energies, gibbs correction of each conformer/tautomer for the protonated(deprotonated) 
        # corresponding to the entry in labels file compn with the hith level of theory 
        ...
        protonated_molecules_energies=protonated_molecules_gibbs_correction+protonated_sp_energies
        deprotonated_molecules_energies=deprotonated_molecules_gibbs_correction+deprotonated_sp_energies
        protonated_molecules_zero_point_energies=protonated_molecules_zero_point_correction+protonated_sp_energies
        deprotonated_molecules_zero_point_energies=deprotonated_molecules_zero_point_correction+deprotonated_sp_energies
        """
        #list of the populations of each conformer/tautomer for the protonated(deprotonated) 
        # corresponding to the entry in labels file compn with the hith level of theory 
        if weighting=="gibbs":
            protonated_molecules_populations=  np.exp(  -627.5095*(protonated_molecules_energies-np.min(protonated_molecules_energies))/(0.594))
            deprotonated_molecules_populations=  np.exp(  -627.5095*(deprotonated_molecules_energies-np.min(deprotonated_molecules_energies))/(0.594))
        elif weighting=="zero":
            protonated_molecules_populations=  np.exp(  -627.5095*(protonated_molecules_zero_point_energies-np.min(protonated_molecules_zero_point_energies))/(0.594))
            deprotonated_molecules_populations=  np.exp(  -627.5095*(deprotonated_molecules_zero_point_energies-np.min(deprotonated_molecules_zero_point_energies))/(0.594))  
        elif weighting=="sp":
            protonated_molecules_populations=  np.exp(  -627.5095*(protonated_sp_energies-np.min(protonated_sp_energies))/(0.594))
            deprotonated_molecules_populations=  np.exp(  -627.5095*(deprotonated_sp_energies-np.min(deprotonated_sp_energies))/(0.594))             
        """
        ...
        deprotonated_gibbs_free_energies_optz= np.array([m.gibbs_free_energy+ref_DG for m in deprotonated_molecules])    * hartrees_to_kal_mol
        protonated_zero_point_energies_optz=   np.array([m.zero_point_energy for m in protonated_molecules])             * hartrees_to_kal_mol
        deprotonated_zero_point_energies_optz= np.array([m.zero_point_energy+ref_zpE for m in deprotonated_molecules])   * hartrees_to_kal_mol
        protonated_sp_energies_optz=           np.array([m.electronic_energy for m in protonated_molecules])             * hartrees_to_kal_mol
        deprotonated_sp_energies_optz=         np.array([m.electronic_energy+ref_SP for m in deprotonated_molecules])    * hartrees_to_kal_mol
        protonated_sp_energies=        np.array([m.electronic_energy for m in protonated_molecules_HL])          * hartrees_to_kal_mol
        deprotonated_sp_energies=      np.array([m.electronic_energy+ref_SP for m in deprotonated_molecules_HL]) * hartrees_to_kal_mol
        ...
        #using orca file
        electronic_spatial_extent_protonated[compn]   =   np.array([ np.sum(np.array(m.properties["quadrupole_moment"][1][1:4],dtype=np.float32) ) for m in protonated_molecules_HL]).dot(protonated_molecules_populations)
        electronic_spatial_extent_deprotonated[compn] =   np.array([ np.sum(np.array(m.properties["quadrupole_moment"][1][1:4],dtype=np.float32) ) for m in deprotonated_molecules_HL]).dot(deprotonated_molecules_populations)
        electronic_spatial_extent_difference[compn]   =   electronic_spatial_extent_protonated[compn] - electronic_spatial_extent_deprotonated[compn]

        total_spatial_extent_protonated[compn]   =   np.array([ np.sum(np.array(m.properties["quadrupole_moment"][2][1:4],dtype=np.float32) ) for m in protonated_molecules_HL]).dot(protonated_molecules_populations)
        total_spatial_extent_deprotonated[compn] =   np.array([ np.sum(np.array(m.properties["quadrupole_moment"][2][1:4],dtype=np.float32) ) for m in deprotonated_molecules_HL]).dot(deprotonated_molecules_populations)
        total_spatial_extent_difference[compn]   =   total_spatial_extent_protonated[compn] - total_spatial_extent_deprotonated[compn]
        ...
        if counter%10==0 or counter==len(rows): 
            ...
            labels["molecular weight"]=protonated_molecular_weight

            labels["protonated charge"]=protonated_charge
            labels["deprotonated charge"]=deprotonated_charge
            #delta zero point energy
            labels["deltaZPE"]=delta_zero_point_energy
            #delta electronic energy
            labels["deltaE"]=delta_electronic_energy
            #delta gibbs free energy
            labels["deltaG"]=delta_energy

            #protonated SMD energy
            labels["protonated SMD-solv"]=SMD_energy_protonated
            labels["deprotonated SMD-solv"]=SMD_energy_deprotonated
            labels["SMD-solv"]=SMD_energy_difference

            #
            labels["protonated NBO-HB"]=NBO_HB_energy_protonated
            labels["deprotonated NBO-HB"]=NBO_HB_energy_deprotonated
            labels["NBO-HB"]=NBO_HB_energy_difference

            labels["protonated expl1wat"]=one_water_stabilization_energy_protonated
            labels["deprotonated expl1wat"]=one_water_stabilization_energy_deprotonated
            labels["expl1wat"]=one_water_stabilization_energy_difference
            ...
            labels["protonated RDG%HB"]=RDG_HB_fraction_protonated*100
            labels["deprotonated RDG%HB"]=RDG_HB_fraction_deprotonated*100
            labels["RDG%HB"]=RDG_HB_fraction_difference*100

            labels["protonated RDG%VdW"]=RDG_vdw_fraction_protonated*100
            labels["deprotonated RDG%VdW"]=RDG_vdw_fraction_deprotonated*100
            labels["RDG%VdW"]=RDG_vdw_fraction_difference*100

            labels["protonated RDG%st"]=RDG_steric_fraction_protonated*100
            labels["deprotonated RDG%st"]=RDG_steric_fraction_deprotonated*100
            labels["RDG%st"]=RDG_steric_fraction_difference*100

            labels["protonated prom-RDG%HB"]=RDG_promol_HB_fraction_protonated*100
            labels["deprotonated prom-RDG%HB"]=RDG_promol_HB_fraction_deprotonated*100
            labels["prom-RDG%HB"]=RDG_promol_HB_fraction_difference*100

            labels["protonated prom-RDG%VdW"]=RDG_promol_vdw_fraction_protonated*100
            labels["deprotonated prom-RDG%VdW"]=RDG_promol_vdw_fraction_deprotonated*100
            labels["prom-RDG%VdW"]=RDG_promol_vdw_fraction_difference*100

            labels["protonated prom-RDG%st"]=RDG_promol_steric_fraction_protonated*100
            labels["deprotonated prom-RDG%st"]=RDG_promol_steric_fraction_deprotonated*100
            labels["prom-RDG%st"]=RDG_promol_steric_fraction_difference*100

            labels["protonated *mu"],labels["deprotonated *mu"]=   dipole_moment_protonated,dipole_moment_deprotonated
            labels["*mu"]=dipole_moment_difference
            labels["protonated tr-e*theta"],labels["deprotonated tr-e*theta"]= electronic_spatial_extent_protonated,electronic_spatial_extent_deprotonated
            labels["tr-e*theta"]=electronic_spatial_extent_difference
            labels["protonated tr-*theta"],labels["deprotonated tr-*theta"]= total_spatial_extent_protonated,total_spatial_extent_deprotonated
            labels["tr-*theta"]=total_spatial_extent_difference

            labels["protonated tr-*alpha"],labels["deprotonated tr-*alpha"]= polarizability_protonated,polarizability_deprotonated
            labels["tr-*alpha"]=polarizability_difference

            labels["protonated HLgap"],labels["deprotonated HLgap"]=HOMO_LUMO_gap_protonated,HOMO_LUMO_gap_deprotonated
            labels["HLgap"]=HOMO_LUMO_gap_difference

            labels["protonated Vol"],labels["deprotonated Vol"],labels["Vol"]=QAMS_volume_protonated,QAMS_volume_deprotonated,QAMS_volume_difference
            labels["protonated Surf"],labels["deprotonated Surf"],labels["Surf"]=QAMS_surface_protonated,QAMS_surface_deprotonated,QAMS_surface_difference
            labels["protonated Surf+"],labels["deprotonated Surf+"],labels["Surf+"]=QAMS_pos_surface_protonated,QAMS_pos_surface_deprotonated,QAMS_pos_surface_difference
            labels["protonated Surf-"],labels["deprotonated Surf-"],labels["Surf-"]=QAMS_neg_surface_protonated,QAMS_neg_surface_deprotonated,QAMS_neg_surface_difference
            labels["protonated min-ESP"],labels["deprotonated min-ESP"],labels["min-ESP"]=QAMS_min_potential_protonated,QAMS_min_potential_deprotonated,QAMS_min_potential_difference            
            labels["protonated max-ESP"],labels["deprotonated max-ESP"],labels["max-ESP"]=QAMS_max_potential_protonated,QAMS_max_potential_deprotonated,QAMS_max_potential_difference            
            labels["protonated avg-ESP"],labels["deprotonated avg-ESP"],labels["avg-ESP"]=QAMS_avg_potential_protonated,QAMS_avg_potential_deprotonated,QAMS_avg_potential_difference
            labels["protonated avg-ESP+"],labels["deprotonated avg-ESP+"],labels["avg-ESP+"]=QAMS_pos_avg_potential_protonated,QAMS_pos_avg_potential_deprotonated,QAMS_pos_avg_potential_difference
            labels["protonated avg-ESP-"],labels["deprotonated avg-ESP-"],labels["avg-ESP-"]=QAMS_neg_avg_potential_protonated,QAMS_neg_avg_potential_deprotonated,QAMS_neg_avg_potential_difference
            labels["protonated var-ESP"],labels["deprotonated var-ESP"],labels["var-ESP"]=QAMS_var_potential_protonated,QAMS_var_potential_deprotonated,QAMS_var_potential_difference
            labels["protonated var-ESP+"],labels["deprotonated var-ESP+"],labels["var-ESP+"]=QAMS_pos_var_potential_protonated,QAMS_pos_var_potential_deprotonated,QAMS_pos_var_potential_difference
            labels["protonated var-ESP-"],labels["deprotonated var-ESP-"],labels["var-ESP-"]=QAMS_neg_var_potential_protonated,QAMS_neg_var_potential_deprotonated,QAMS_neg_var_potential_difference
            labels["protonated *PI-ESP"],labels["deprotonated *PI-ESP"],labels["*PI-ESP"]=QAMS_PI_protonated,QAMS_PI_deprotonated,QAMS_PI_difference
            labels["protonated MPI"],labels["deprotonated MPI"],labels["MPI"]=QAMS_MPI_protonated,QAMS_MPI_deprotonated,QAMS_MPI_difference

            labels["protonated SMDvol"],labels["deprotonated SMDvol"],labels["SMDvol"]=SMD_volume_protonated,SMD_volume_deprotonated,SMD_volume_difference
            labels["protonated SMDsurf"],labels["deprotonated SMDsurf"],labels["SMDsurf"]=SMD_surface_protonated,SMD_surface_deprotonated,SMD_surface_difference
            labels["protonated SMDsurf+"],labels["deprotonated SMDsurf+"],labels["SMDsurf+"]=SMD_pos_surface_protonated,SMD_pos_surface_deprotonated,SMD_pos_surface_difference
            labels["protonated SMDsurf-"],labels["deprotonated SMDsurf-"],labels["SMDsurf-"]=SMD_neg_surface_protonated,SMD_neg_surface_deprotonated,SMD_neg_surface_difference
            labels["protonated SMD-min-ESP"],labels["deprotonated SMD-min-ESP"],labels["SMD-min-ESP"]=SMD_min_potential_protonated,SMD_min_potential_deprotonated,SMD_min_potential_difference            
            labels["protonated SMD-max-ESP"],labels["deprotonated SMD-max-ESP"],labels["SMD-max-ESP"]=SMD_max_potential_protonated,SMD_max_potential_deprotonated,SMD_max_potential_difference            
            labels["protonated SMD-avg-ESP"],labels["deprotonated SMD-avg-ESP"],labels["SMD-avg-ESP"]=SMD_avg_potential_protonated,SMD_avg_potential_deprotonated,SMD_avg_potential_difference
            labels["protonated SMD-avg-ESP+"],labels["deprotonated SMD-avg-ESP+"],labels["SMD-avg-ESP+"]=SMD_pos_avg_potential_protonated,SMD_pos_avg_potential_deprotonated,SMD_pos_avg_potential_difference
            labels["protonated SMD-avg-ESP-"],labels["deprotonated SMD-avg-ESP-"],labels["SMD-avg-ESP-"]=SMD_neg_avg_potential_protonated,SMD_neg_avg_potential_deprotonated,SMD_neg_avg_potential_difference
            labels["protonated SMD-var-ESP"],labels["deprotonated SMD-var-ESP"],labels["SMD-var-ESP"]=SMD_var_potential_protonated,SMD_var_potential_deprotonated,SMD_var_potential_difference
            labels["protonated SMD-var-ESP+"],labels["deprotonated SMD-var-ESP+"],labels["SMD-var-ESP+"]=SMD_pos_var_potential_protonated,SMD_pos_var_potential_deprotonated,SMD_pos_var_potential_difference
            labels["protonated SMD-var-ESP-"],labels["deprotonated SMD-var-ESP-"],labels["SMD-var-ESP-"]=SMD_neg_var_potential_protonated,SMD_neg_var_potential_deprotonated,SMD_neg_var_potential_difference
            labels["protonated SMD-*PI-ESP"],labels["deprotonated SMD-*PI-ESP"],labels["SMD-pi-ESP"]=SMD_PI_protonated,SMD_PI_deprotonated,SMD_PI_difference
            labels["protonated SMD-MPI"],labels["deprotonated SMD-MPI"],labels["SMD-dMPI"]=SMD_MPI_protonated,SMD_MPI_deprotonated,SMD_MPI_difference
            ...
            labels["protonated min-LEA"],labels["deprotonated min-LEA"],labels["min-LEA"]=LEA_min_potential_protonated,LEA_min_potential_deprotonated,LEA_min_potential_difference            
            labels["protonated max-LEA"],labels["deprotonated max-LEA"],labels["max-LEA"]=LEA_max_potential_protonated,LEA_max_potential_deprotonated,LEA_max_potential_difference            
            labels["protonated avg-LEA"],labels["deprotonated avg-LEA"],labels["avg-LEA"]=LEA_avg_potential_protonated,LEA_avg_potential_deprotonated,LEA_avg_potential_difference
            ...
            labels["protonated var-LEA"],labels["deprotonated var-LEA"],labels["var-LEA"]=LEA_var_potential_protonated,LEA_var_potential_deprotonated,LEA_var_potential_difference
            ...
            labels["protonated min-ALIE"],labels["deprotonated min-ALIE"],labels["min-ALIE"]=ALIE_min_potential_protonated,ALIE_min_potential_deprotonated,ALIE_min_potential_difference            
            labels["protonated max-ALIE"],labels["deprotonated max-ALIE"],labels["max-ALIE"]=ALIE_max_potential_protonated,ALIE_max_potential_deprotonated,ALIE_max_potential_difference            
            labels["protonated avg-ALIE"],labels["deprotonated avg-ALIE"],labels["avg-ALIE"]=ALIE_avg_potential_protonated,ALIE_avg_potential_deprotonated,ALIE_avg_potential_difference
            labels["protonated var-ALIE"],labels["deprotonated var-ALIE"],labels["var-ALIE"]=ALIE_var_potential_protonated,ALIE_var_potential_deprotonated,ALIE_var_potential_difference

            labels["protonated avg|EF|"],labels["deprotonated avg|EF|"]=V_grad_average_protonated,V_grad_average_deprotonated
            labels["avg|EF|"]=V_grad_average_protonated-V_grad_average_deprotonated
            labels["protonated avgEF*tang"],labels["deprotonated avgEF*tang"]=V_grad_tang_average_protonated,V_grad_tang_average_deprotonated
            labels["avgEF*tang"]=V_grad_tang_average_protonated-V_grad_tang_average_deprotonated
            labels["protonated avgEF*norm"],labels["deprotonated avgEF*norm"]=V_grad_norm_average_protonated,V_grad_norm_average_deprotonated
            labels["avgEF*norm"]=V_grad_norm_average_protonated-V_grad_norm_average_deprotonated

            labels["protonated 0.95q|EF|"],labels["deprotonated 0.95q|EF|"]=V_grad_tang_95_protonated,V_grad_tang_95_deprotonated
            labels["0.95q|EF|"]=V_grad_95_protonated-V_grad_95_deprotonated
            labels["protonated 0.95qEF*tang"],labels["deprotonated 0.95qEF*tang"]=V_grad_tang_95_protonated,V_grad_tang_95_deprotonated
            labels["0.95qEF*tang"]=V_grad_tang_95_protonated-V_grad_tang_95_deprotonated
            labels["protonated 0.95qEF*norm"],labels["deprotonated 0.95qEF*norm"]=V_grad_norm_95_protonated,V_grad_norm_95_deprotonated  
            labels["0.95qEF*norm"]=V_grad_norm_95_protonated-V_grad_norm_95_deprotonated       

            labels["protonated 0.9q|EF|"],labels["deprotonated 0.9q|EF|"]=V_grad_tang_90_protonated,V_grad_tang_90_deprotonated
            labels["0.9q|EF|"]=V_grad_90_protonated-V_grad_95_deprotonated
            labels["protonated 0.9qEF*tang"],labels["deprotonated 0.9qEF*tang"]=V_grad_tang_90_protonated,V_grad_tang_90_deprotonated
            labels["0.9qEF*tang"]=V_grad_tang_90_protonated-V_grad_tang_95_deprotonated
            labels["protonated 0.9qEF*norm"],labels["deprotonated 0.9qEF*norm"]=V_grad_norm_90_protonated,V_grad_norm_90_deprotonated  
            labels["0.9qEF*norm"]=V_grad_norm_90_protonated-V_grad_norm_95_deprotonated  

            labels["protonated 0.75q|EF|"],labels["deprotonated 0.75q|EF|"]=V_grad_tang_75_protonated,V_grad_tang_75_deprotonated
            labels["0.75q|EF|"]=V_grad_75_protonated-V_grad_75_deprotonated
            labels["protonated 0.75qEF*tang"],labels["deprotonated 0.75qEF*tang"]=V_grad_tang_75_protonated,V_grad_tang_75_deprotonated
            labels["0.75qEF*tang"]=V_grad_tang_75_protonated-V_grad_tang_75_deprotonated
            labels["protonated 0.75qEF*norm"],labels["deprotonated 0.75qEF*norm"]=V_grad_norm_75_protonated,V_grad_norm_75_deprotonated  
            labels["0.75qEF*norm"]=V_grad_norm_75_protonated-V_grad_norm_75_deprotonated   

            labels["protonated 0.5q|EF|"],labels["deprotonated 0.5q|EF|"]=V_grad_50_protonated,V_grad_50_deprotonated
            labels["0.5q|EF|"]=V_grad_50_protonated-V_grad_50_deprotonated
            labels["protonated 0.5qEF*tang"],labels["deprotonated 0.5qEF*tang"]=V_grad_tang_50_protonated,V_grad_tang_50_deprotonated
            labels["0.5qEF*tang"]=V_grad_tang_50_protonated-V_grad_tang_50_deprotonated
            labels["protonated 0.5qEF*norm"],labels["deprotonated 0.5qEF*norm"]=V_grad_norm_50_protonated,V_grad_norm_50_deprotonated  
            labels["0.5qEF*norm"]=V_grad_norm_50_protonated-V_grad_norm_50_deprotonated 

            labels["protonated avgEF*angle"],labels["deprotonated avgEF*angle"]=V_grad_ang_average_protonated,V_grad_ang_average_deprotonated
            labels["avgEF*angle"]=V_grad_ang_average_protonated-V_grad_ang_average_deprotonated
            labels["protonated 0.95qEF*angle"],labels["deprotonated 0.95qEF*angle"]=V_grad_ang_95_protonated,V_grad_ang_95_deprotonated
            labels["0.95qEF*angle"]=V_grad_ang_95_protonated-V_grad_ang_95_deprotonated
            labels["protonated 0.9qEF*angle"],labels["deprotonated 0.9qEF*angle"]=V_grad_ang_90_protonated,V_grad_ang_90_deprotonated
            labels["0.9qEF*angle"]=V_grad_ang_90_protonated-V_grad_ang_90_deprotonated            
            labels["protonated 0.75qEF*angle"],labels["deprotonated 0.75qEF*angle"]=V_grad_ang_75_protonated,V_grad_ang_75_deprotonated
            labels["0.75qEF*angle"]=V_grad_ang_75_protonated-V_grad_ang_75_deprotonated
            labels["protonated 0.5qEF*angle"],labels["deprotonated 0.5qEF*angle"]=V_grad_ang_50_protonated,V_grad_ang_50_deprotonated
            labels["0.5qEF*angle"]=V_grad_ang_50_protonated-V_grad_ang_50_deprotonated

            labels["protonated avg|EF|/V"],labels["deprotonated avg|EF|/V"]=V_grad_red_average_protonated,V_grad_red_average_deprotonated
            labels["avg|EF|/V"]=V_grad_red_average_protonated-V_grad_red_average_deprotonated
            labels["protonated avgEF*tang/V"],labels["deprotonated avgEF*tang/V"]=V_grad_tang_red_average_protonated,V_grad_tang_red_average_deprotonated
            labels["avgEF*tang/V"]=V_grad_tang_red_average_protonated-V_grad_tang_red_average_deprotonated
            labels["protonated avgEF*norm/V"],labels["deprotonated avgEF*norm/V"]=V_grad_norm_red_average_protonated,V_grad_norm_red_average_deprotonated
            labels["avgEF*norm/V"]=V_grad_norm_red_average_protonated-V_grad_norm_red_average_deprotonated
            labels["protonated 0.95q|EF|/V"],labels["deprotonated 0.95q|EF|/V"]=V_grad_red_95_protonated,V_grad_red_95_deprotonated
            labels["0.95q|EF|/V"]=V_grad_red_95_protonated-V_grad_red_95_deprotonated
            labels["protonated 0.95qEF*tang/V"],labels["deprotonated 0.95qEF*tang/V"]=V_grad_tang_red_95_protonated,V_grad_tang_red_95_deprotonated
            labels["0.95qEF*tang/V"]=V_grad_tang_red_95_protonated-V_grad_tang_red_95_deprotonated
            labels["protonated 0.95qEF*norm/V"],labels["deprotonated 0.95qEF*norm/V"]=V_grad_norm_red_95_protonated,V_grad_norm_red_95_deprotonated
            labels["0.95qEF*norm/V"]=V_grad_norm_red_95_protonated-V_grad_norm_red_95_deprotonated
            labels["protonated 0.9q|EF|/V"],labels["deprotonated 0.9q|EF|/V"]=V_grad_red_90_protonated,V_grad_red_90_deprotonated
            labels["0.9q|EF|/V"]=V_grad_red_90_protonated-V_grad_red_90_deprotonated
            labels["protonated 0.9qEF*tang/V"],labels["deprotonated 0.9qEF*tang/V"]=V_grad_tang_red_90_protonated,V_grad_tang_red_90_deprotonated
            labels["0.9qEF*tang/V"]=V_grad_tang_red_90_protonated-V_grad_tang_red_90_deprotonated
            labels["protonated 0.9qEF*norm/V"],labels["deprotonated 0.9qEF*norm/V"]=V_grad_norm_red_90_protonated,V_grad_norm_red_90_deprotonated
            labels["0.9qEF*norm/V"]=V_grad_tang_red_90_protonated-V_grad_tang_red_90_deprotonated
            labels["protonated 0.75q|EF|/V"],labels["deprotonated 0.75q|EF|/V"]=V_grad_red_75_protonated,V_grad_red_75_deprotonated
            labels["0.75q|EF|/V"]=V_grad_red_75_protonated-V_grad_red_75_deprotonated
            labels["protonated 0.75qEF*tang/V"],labels["deprotonated 0.75qEF*tang/V"]=V_grad_tang_red_75_protonated,V_grad_tang_red_75_deprotonated
            labels["0.75qEF*tang/V"]=V_grad_tang_red_75_protonated-V_grad_tang_red_75_deprotonated
            labels["protonated 0.75qEF*norm/V"],labels["deprotonated 0.75qEF*norm/V"]=V_grad_norm_red_75_protonated,V_grad_norm_red_75_deprotonated
            labels["0.75qEF*norm/V"]=V_grad_norm_red_75_protonated-V_grad_norm_red_75_deprotonated
            labels["protonated 0.5qEF*mod/V"],labels["deprotonated 0.5qEF*mod/V"]=V_grad_red_50_protonated,V_grad_red_50_deprotonated
            labels["0.5qEF*mod/V"]=V_grad_red_50_protonated-V_grad_red_50_deprotonated
            labels["protonated 0.5qEF*tang/V"],labels["deprotonated 0.5qEF*tang/V"]=V_grad_tang_red_50_protonated,V_grad_tang_red_50_deprotonated
            labels["0.5qEF*tang/V"]=V_grad_tang_red_50_protonated-V_grad_tang_red_50_deprotonated
            labels["protonated 0.5qEF*norm/V"],labels["deprotonated 0.5qEF*norm/V"]=V_grad_norm_red_50_protonated,V_grad_norm_red_50_deprotonated
            labels["0.5qEF*norm/V"]=V_grad_norm_red_50_protonated-V_grad_norm_red_50_deprotonated


            print ("writing to file: "+destination+values_extracted_file+"                                     ")
            labels.to_csv(destination+values_extracted_file)
