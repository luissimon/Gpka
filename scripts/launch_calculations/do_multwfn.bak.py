...
import os.path
import sys
#sys.path.append('../import') #required to import Molecular_structure
import time
...
def create_molden_input(filename):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    command="/home/orca5/orca_2mkl "+filename+" -molden "
    os.system(command)
...
#charges
def multwfn_chrg(filename,method,n_proc):  #method: hirshfeld,voronoy,mulliken,lowdin,becke,ADCH,CHELPG,MK,CM5,12CM5,RESP,PEOE
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input < "+method+"-chrg.multiwfn > null  -nt "+str(n_proc)
    os.system(command)
    with open(filename+".molden.chg","r") as f: charges=[float(line.split()[4]) for line in f.readlines()]
    return charges

#bond orders
def multwfn_BO(filename,method,n_proc): #method: mayer,wiberg,mulliken,fuzzy,laplacian
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input < "+method+"-BO.multiwfn > null  -nt "+str(n_proc)
    print (method)
    os.system(command)
...
def multwfn_IBSI_BO(filename,n_proc):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    xyz_file=filename+".molden.xyz"
    os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input < IBSI-BO.multiwfn > IBSI.txt  -nt "+str(n_proc)
    os.system(command)
...
def multwfn_expensive_IBSI_BO(filename,n_proc):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    xyz_file=filename+".molden.xyz"
    os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input < expensive_IBSI-BO.multiwfn > IBSI.txt  -nt "+str(n_proc)
    os.system(command)
...
def multwfn_QAMS(filename,n_proc):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    xyz_file=filename+".molden.xyz"
    os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input <QAMS.multiwfn> QAMS.txt  -nt "+str(n_proc)
    os.system(command)
...
def multwfn_FASA(filename,n_proc):
        if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
        os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
        command="/home/multiwfn_3.7/Multiwfn "+filename+".molden.input <FASA-atoms.multiwfn> null  -nt "+str(n_proc)
        os.system(command)
...
def multwfn_AIM(filename,n_proc):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    xyz_file=filename+".molden.xyz"
    os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input <AIM.multiwfn> null  -nt "+str(n_proc)
    os.system(command)
...
def multwfn_v_at_nucleus(filename):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    v_at_nucl={}
    vs=[]
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input < get_v_at_nucleus.multiwfn > v_at_nucleus.txt  -nt "+str(n_proc)
    print (command)
    os.system(command)
...
def rdg_analysis_weak_inter(filename,lim_weak=0.5,lim_h_bonds=-0.01,lim_steric=0.01,promolecular=False):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]

    if promolecular==False:
        command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input < rdg_plot.multiwfn > null.txt  -nt "+str(n_proc)
    else: 
        command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input < rdg_promol_plot.multiwfn > null.txt  -nt "+str(n_proc)
    os.system(command)
...
#molecular properties:
def multwfn_ALIE(filename,n_proc):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    xyz_file=filename+".molden.xyz"
    os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input <ALIE.multiwfn> ALIE.txt  -nt "+str(n_proc)
    os.system(command)
...
#molecular properties:
def multwfn_LEA(filename,n_proc):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    xyz_file=filename+".molden.xyz"
    os.environ["LEA"]="/home/multiwfn_3.8"
    command="/home/multiwfn_3.8/Multiwfn "+filename+".molden.input <LEA.multiwfn> LEA.txt  -nt "+str(n_proc)
    os.system(command)
...
def extract_features(filename,n_proc=14,req_features=[]):
    if filename[-4:]==".gbw": filename=filename.split(".gbw")[0]
    beg_time=time.time()
    if not os.path.isfile(filename.split(".gbw")[0]+".molden.input"):
        create_molden_input(filename)
    ...
    for method in chg_methods: 
        features["chg_"+method]=multwfn_chrg(filename,method,n_proc)

    #bo_methods=["mayer","wiberg","mulliken","fuzzy","laplacian"]
    for method in bo_methods: 
        features["bo_"+method]=multwfn_BO(filename,method,n_proc)

    if req_features==[] or "bo_IBSI" in req_features:
        features["bo_IBSI"]=multwfn_IBSI_BO(filename,n_proc)

    if req_features==[] or "qams" in req_features:
        features.update(multwfn_QAMS(filename,n_proc))

    if req_features==[] or "fasa" in req_features:
        features.update(multwfn_FASA(filename,n_proc))
    ...
    if "aim" in req_features:
        features.update(multwfn_AIM(filename,n_proc))

    if req_features==[] or "v_at_nucl" in req_features:
        features.update(multwfn_v_at_nucleus (filename))
    ...
    if req_features==[] or "ALIE" in req_features:
        features.update(multwfn_ALIE(filename,n_proc))

    if req_features==[] or "LEA" in req_features:
        features.update(multwfn_LEA(filename,n_proc))

    if req_features==[] or "rdg" in req_features:
        features.update(rdg_analysis_weak_inter(filename,promolecular=False))

    if req_features==[] or "rdg_promolecular" in req_features:
        features.update(rdg_analysis_weak_inter(filename,promolecular=True))
...
if __name__ == "__main__":
    ...
    req_features=sys.argv[3:]
    if req_features==["all"]: req_features=[]
    directory="/".join(gbw_file.split("/")[:-1])
    file=gbw_file.split("/")[-1]
    os.chdir(directory)
    os.environ["Multiwfnpath"]="/home/multiwfn_3.8"
    extract_features(file,req_features=req_features,n_proc=n_proc)
