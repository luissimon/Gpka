...
def water_effect(file_name,n_waters=1,n_procs=14,scratch_dir="/scratch/lsimon/"):
    ...
    #options="--gfn2 --nopreopt --gbsa h2o --nofix --shake 0 --tstep 0.01"
...
if __name__ == "__main__":
    ...
    scratch=os.path.join(scratch_home,xyz_file[:-4])
    if not os.path.isdir(scratch):os.mkdir(scratch)
    #inp_sp_files=prepare_sp_files(opt_out_file,n_procs=n_procs,levels_of_theory=["pbeh3c","m06","wb97xd","swb97xd","sm06"])
    inp_sp_files=prepare_sp_files(opt_out_file,n_procs=n_procs,levels_of_theory=["pbeh3c","m06","sm06","wb97xd","swb97xd"])
    #inp_sp_files=prepare_sp_files(opt_out_file,n_procs=n_procs,levels_of_theory=["m06"])

    #inp_sp_files=prepare_sp_files(opt_out_file,n_procs=n_procs,levels_of_theory=["swb97xd"])
