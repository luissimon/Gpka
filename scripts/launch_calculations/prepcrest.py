...
def prep_inp_from_xyz(mxyz_file,head,filename,first=0):
    ...
    for xyz in xyzs[1:]:
        i+=1
        text=head+xyz+"end\n"
        files.append(filename+str(i)+".inp")
...
if "-tautomerize" not in sys.argv: files= prep_inp_from_xyz("crest_conformers.xyz",head,xyz_file_name,first) 
elif "-tautomerize" in sys.argv: files= prep_inp_from_xyz("./"+xyz_file_name+"_crest/tautomers.xyz",head,xyz_file_name,first)
for f in files:
    command2="/home/scripts/qorca5.py "+f  #+" -qp 1  " 
    print (command2)
    #os.system(command2)
