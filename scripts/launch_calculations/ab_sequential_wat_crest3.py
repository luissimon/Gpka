...
def run_crest_qgc(xyz_file="temp.xyz", options="--gfn2 --gbsa h2o",n_waters=1,nprocs=14,chrg=0):
    ...
    command="/bin/crest3 "+str(xyz_file)+" --qcg water.xyz --nsolv "+str(n_waters)+" --T "+str(nprocs)+" --chrg "+str(chrg)+" --ensemble "+options+ " > crest_qgc.out"
    print (command)
    os.system(command)
...
def water_effect(file_name,n_waters=1,n_procs=14,scratch_dir="/scratch/lsimon/"):
    ...
    options="--gfnff --enslvl gfnff --fin_opt_gfn2 --nopreopt --fixsolute  --gbsa h2o"
