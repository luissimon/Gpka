...
class composed_regressor(sklearn.base.BaseEstimator):
    ...
    def set_params(self,**kwargs):
        ...
        if self.non_linear_regressor_name in ["BagginRegressor","BaggingRegressor-XGB","BaggingRegressor-XGB-hist","BaggingRegressor-LGBM"]:
            self.non_lin_reg.estimator.set_params(**default_estimator_non_linear_params)
        super().set_params()
    ...
    def do_permutation_importances(self, X_train,y_train,fraction=1.0,verbose=True):
        """
        TO TEST!!!
    ...
    def fit(self,X_train,y_train,sample_weight=None,early_stopping_rounds=10):
        ...
        try: self.lin_reg.fit(self.linear_X_fitted,y_train,sample_weight=sample_weight)
            ...
            print (ValueError)
        if self.fit_failure!=True and self.linear_regressor_name not in ["LinearRegression","RANSACRegressor","ElasticNet"] and self.lin_reg.n_iter_==self.lin_reg.get_params()["max_iter"]:
