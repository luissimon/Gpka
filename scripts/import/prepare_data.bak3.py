...
def standarize_data(train_data,test_data,all_data,not_standarize=[]):
    ...
    for trd in train_data:
        if trd not in not_standarize: 
            ...
            for d in [test_data,all_data]:
                if d is not None:
                    tt=np.asarray(d[trd]).reshape(-1,1)
                    d[trd]=standard_scalers[trd].transform(tt) 
...
def prepare_eq_data(file_name,drop_compounds,test_size=0.2,correlated_groups={},test_suffix="_std_test.csv",train_suffix="_std_train.csv",all_suffix="_std_all.csv",standarize=True,prepare_test_set=True):
    ...
    for d in force_in_train_set_compounds: 
        ...
        data =data[data["compn"].str.startswith(d)==False]
    df=data[attributes_for_clustering]#borrame
    print(df.applymap(np.isreal).all(1).to_string())#borrame
    print(df.columns[df.isnull().any()])
    print(df[df.isnull().any(axis=1)])
    print(df[~df.applymap(np.isreal).all(1)])#borrame
    print(np.argmin(df.applymap(np.isreal).all(1)))#borrame
    ...
    if standarize:
        ...
        train_data,test_data,all_data=standarize_data(train_data,test_data,data,not_standarize=not_standarize)

    #save data:
    train_data.to_csv(train_data_file_name)
    if test_data is not None: 
        test_data.to_csv(test_data_file_name)
        data.to_csv(all_data_file_name)

def prepare_graph_data_to_ML(json_file,csv_file_name,correlated_groups,test_suffix="_std_test.csv",train_suffix="_std_train.csv",prepare_test_set=True,standarize=True):
    ...
    if len(transformed_eq_features_replace)>0:     train_dataset.aply_mask_to_atom_features( transformed_eq_features_replace, replace=True )
    if len(transformed_eq_features_no_replace)>0:  train_dataset.aply_mask_to_atom_features( transformed_eq_features_no_replace,replace=False)
    train_dataset.features_n_bonds_away(suffixes=["alpha","beta","gamma"],exclude_ending="_H_difference")
    train_dataframe=train_dataset.eq_features_to_pd_series()
    ...
    if test_suffix!="" or prepare_test_set:
        test_dataset=gpka_spektral_dataset.gpka_spektral_dataset(json_file,csv_file=test_data_file_name,equilibrium_keys=categorical_features+eq_features,
                                linear_equilibrium_keys=linear_eq_features,label_key="pKa")  
        if len(transformed_eq_features_replace)>0:     test_dataset.aply_mask_to_atom_features( transformed_eq_features_replace, replace=True )
        if len(transformed_eq_features_no_replace)>0:  test_dataset.aply_mask_to_atom_features( transformed_eq_features_no_replace,replace=False)
        test_dataset.features_n_bonds_away(suffixes=["alpha","beta","gamma"],exclude_ending="_H_difference")
        test_dataframe=test_dataset.eq_features_to_pd_series()
    ...
    else: test_dataframe=None

    if standarize:
        not_standarize=linear_eq_features+["compn"]+["pKa"]+categorical_features+["cluster_index"]
        train_dataframe,test_dataframe,_= standarize_data(train_dataframe,test_dataframe,None,not_standarize=not_standarize)

    train_dataframe.to_csv(train_data_new_file_name[:-4]+"_with_graph_data.csv",index=False)
