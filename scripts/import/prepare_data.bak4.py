...
def prepare_eq_data(file_name,drop_compounds,test_size=0.2,correlated_groups={},prepare_test_set=True,
    ...
    if test_size>0 and prepare_test_set:
        ...
        train_data, test_data, all_data = data, None, None

    #standarize the data
    if standarize:
        not_standarize_features=selected_lin_eq_features+["compn"]+["pKa"]+categorical_features+["cluster_index"]
...
def prepare_graph_data_to_ML(json_file,csv_file_name,correlated_groups,
                            test_suffix="_std_test.csv",train_suffix="_std_train.csv",
    ...
    if standarize:
        not_standarize_features=linear_eq_features+["compn"]+["pKa"]+categorical_features+["cluster_index"]
...
def prepare_graph_data(json_file,csv_file_name,correlated_groups,test_suffix="_std_test.csv",train_suffix="_std_train.csv",prepare_test_set=True):
    ...
    for d in prepare_files:  
            ...
            print (dataset.equilibrium_keys)
            print (dataset.atom_vector_keys)
